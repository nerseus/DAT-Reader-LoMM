...
public static class ABCToUnity
{
    ...
    private static Mesh CreateMesh(string modelName, Piece piece)
    {
    ...
    private static Dictionary<string, UnityDTX> LoadSkins(List<string> skinTextures, string projectPath)
    {
        Dictionary<string, UnityDTX> unityDTXModels = new Dictionary<string, UnityDTX>();
        foreach (var skin in skinTextures)
        {
            string filenameAndPathToTexture = Path.Combine(projectPath, skin);
            var dtxModel = DTXReader.LoadDTXModel(filenameAndPathToTexture, skin);
            if (dtxModel == null)
            {
    ...
    public static GameObject CreateObjectFromABC(ABCModel model, List<string> skinTextures, string projectPath, Dictionary<string, UnityDTX> unityDTXModels = null)
    {
    ...
    {
        ABCModel model = ABCModelReader.LoadABCModel(filenameAndPathToABC, projectPath);
        if (model == null)
        {
            return null;
        }
